 ...
 * - App-specific configuration access
 */
export abstract class BasePage {
  protected appName: AppName;
  protected appConfig: import('./../../config/app.config').AppConfig;
  ...
  /**
   * Get app-specific selector from config
   * @param category - Selector category (e.g., 'login')
   * @param element - Element name (e.g., 'usernameInput')
   */
  protected getAppSelector(category: string, element: string): string {
    return this.appConfig.selectors[category]?.[element] || '';
  }
  ...
  async waitForPageLoad(): Promise<void> {
    await waitForNetworkStable(this.page, { maxWaitMs: this.appConfig.timeouts.page });
  }
  ...
  async navigateTo(path: string): Promise<void> {
    await this.page.goto(`${this.appConfig.baseUrl}${path}`);
